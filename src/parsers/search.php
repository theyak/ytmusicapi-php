<?php
    ...
    define('ALL_RESULT_TYPES', ["album", "artist", "playlist", "song", "video", "station", "profile", "podcast", "episode"]);
    define('API_RESULT_TYPES', ["single", "ep", ...ALL_RESULT_TYPES]);
    ...
    function get_search_result_type($result_type_local, $result_types_local)
    {
        ...
        if (!in_array($result_type_local, $result_types_local)) {
            ...
            $result_type = ALL_RESULT_TYPES[array_search($result_type_local, $result_types_local)];
        }
    ...
    function parse_top_result($data, $search_result_types)
    {
        ...
        if ($result_type === 'episode') {
            $search_result["videoId"] = nav($data, join(THUMBNAIL_OVERLAY_NAVIGATION, WATCH_VIDEO_ID));
            $search_result["videoType"] = nav($data, join(THUMBNAIL_OVERLAY_NAVIGATION, NAVIGATION_VIDEO_TYPE));
            $runs = array_slice(nav($data, SUBTITLE_RUNS), 2);
    ...
    function parse_search_result($data, $search_result_types, $result_type, $category)
    {
        ...
        if (!$result_type) {
            ...
            if ($browse_id) {
                ...
                if ($video_type === 'MUSIC_VIDEO_TYPE_ATV') {
                    $result_type = 'song';
                } else if ($video_type === "MUSIC_VIDEO_TYPE_PODCAST_EPISODE") {
                    $result_type = 'episode';
                } else {
                    $result_type = 'video';
                }
        ...
        if ($result_type === 'artist') {
            ...
            $play_navigation = nav($data, join(PLAY_BUTTON, 'playNavigationEndpoint'), true);
        ...
        if (in_array($result_type, ['episode'])) {
            $flex_item = get_flex_column_item($data, 1);
            $runs = array_slice(nav($flex_item, TEXT_RUNS), $default_offset);
            $has_date = intval(count($runs) > 1);
